...
<!-- MS to PG: in the interest of keeping the slides short, should we skip one of the following discussion questions?  
PG to MS I would prefer to keep both. They convey deeply different things, one is creative and one is about taking a dataset and imposing different encodings on it. Feel free to skip though in your lecture if you want-->
...
* quantitative: comparisons of magnitudes "The average age is 17.5"; "The 30-year-old is three times older than the 10-year-old."
...
![](pictures/munzer_fig5.6_1.png){fig-width=80%, fig-align="center"}
...
![](pictures/munzer_fig5.6_2.png){fig-width=80%, fig-align="center"}
...
```{.python style="font-size: 0.6em" code-line-numbers="1|"}
...
alt.Chart(df_latest).mark_bar().encode(
    x=alt.X('country:N',
            =alt.EncodingSortField(field='life_expect', order='descending'sort),
            title='Country'),
...
::: {.notes }
 altair is treating it as nominal. We (as the reader) can see that they can be ranked -- though it's unclear whether the underlying data is actually ordinal.
:::
<!-- PG to MS I don't understand this answer in the speaker notes... it is groups of countries and it should be nominal.

MS to GGG: got rid of the discussion question now that we've fixed cluster-->
...
alt.Chart(data2000).mark_point(filled=True, size=100).encode(
    alt.X('fertility:Q', title = "Fertility (children per woman)"),
    alt.Y('life_expect:Q', scale=alt.Scale(zero=False), title = "Life expectancy"),
    alt.Shape('cluster:N', title = "Cluster")
)
```
...
["Bad"]{style="color:red;"} use of encoding: here is an instance of a "wasted" opportunity to encode something useful on the x-axis
